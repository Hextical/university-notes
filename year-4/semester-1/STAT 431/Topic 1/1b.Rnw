...
\begin{itemize}
    ...
    \item The likelihood ratio based $ \qty{95}{\percent} $ CI is $(0.017, 0.023)$.
          % \begin{noindent}
            ...
            left = uniroot(LRCI, c(0, theta.hat), y = y, n = n)$root
            right = uniroot(LRCI, c(theta.hat, 1), y = y, n = n)$root
            abline(v = left, col = "red", lty = 2)
            abline(v = right, col = "red", lty = 2)
            abline(h = qchisq(0.95,1)*(-1/2), col = "blue")
            axis(1, at = left, labels = round(left, 3), col.axis = "red", las = 2)
            axis(1, at = right, labels = round(right, 3), col.axis = "red", las = 2)
            axis(2, at = qchisq(0.95,1)*(-1/2), labels = round(qchisq(0.95,1)*(-1/2), 2), col.axis = "red", las = 2)
            @
            % \end{noindent}
\end{itemize}
