...
\DeclarePairedDelimiterXPP{\E}[1]{\operatorname{E}}[]{}{%
    \renewcommand\given{\nonscript\:\delimsize\vert\nonscript\:\mathopen{}}%
    \ifblank{#1}{\:\cdot\:}%
    #1}%
\DeclarePairedDelimiterXPP{\V}[1]{\operatorname{V}}(){}{%
    \renewcommand\given{\nonscript\:\delimsize\vert\nonscript\:\mathopen{}}%
    \ifblank{#1}{\:\cdot\:}%
    #1}%
...
\DeclarePairedDelimiterXPP\Prob[1]{\operatorname{P}}(){}{%
    ...
    \ifblank{#1}{\:\cdot\:}%
    #1}%
\DeclarePairedDelimiterXPP\Ind[1]{\operatorname{I}}\{\}{}{%
    \renewcommand\given{\nonscript\:\delimsize\vert\nonscript\:\mathopen{}}%
    \ifblank{#1}{\:\cdot\:}%
    #1}%
\DeclarePairedDelimiterXPP{\se}[1]{\operatorname{se}}(){}{%
    \ifblank{#1}{\:\cdot\:}%
    #1}%
\DeclarePairedDelimiterXPP{\estse}[1]{\widehat{\operatorname{se}}}(){}{%
    \ifblank{#1}{\:\cdot\:}%
    #1}%
\DeclarePairedDelimiterXPP{\estV}[1]{\widehat{\operatorname{V}}}(){}{
    \renewcommand\given{\nonscript\:\delimsize\vert\nonscript\:\mathopen{}}%
    \ifblank{#1}{\:\cdot\:}%
    #1}%
\DeclarePairedDelimiterXPP{\estVar}[1]{\widehat{\operatorname{Var}}}(){}{
    \renewcommand\given{\nonscript\:\delimsize\vert\nonscript\:\mathopen{}}%
    \ifblank{#1}{\:\cdot\:}%
    #1}%
\let\exp\relax%
\let\log\relax%
\let\ln\relax%
\DeclarePairedDelimiterXPP{\exp}[1]{\operatorname{exp}}\{\}{}{#1}%
\DeclarePairedDelimiterXPP{\log}[1]{\operatorname{log}}(){}{#1}%
\DeclarePairedDelimiterXPP{\ln}[1]{\operatorname{ln}}(){}{#1}%
\DeclarePairedDelimiterXPP{\diag}[1]{\operatorname{diag}}(){}{#1}%
\DeclarePairedDelimiterXPP{\sign}[1]{\operatorname{sign}}(){}{#1}%

\DeclarePairedDelimiterXPP{\expit}[1]{\operatorname{expit}}(){}{#1}%
\DeclarePairedDelimiterXPP{\logit}[1]{\operatorname{logit}}(){}{#1}%
...
\end{knitrout}
    \begin{itemize}
        ...
        \item We are using indicator variables for the explanatory variables:
              \begin{align*}
                  x_{i1} & =\text{\lstinline{loc}} & & \text{(1 for Intensive, 0 for Regular)}    \\
                  x_{i2} & =\text{\lstinline{clinic}} & & \text{(1 for Clinic A, 0 for Clinic B)}
              \end{align*}
        \item The variable \lstinline{y} records the number of deaths (events).
    \end{itemize}
\end{Example}
\subsection*{Fit GLMs using R}
The \lstinline{glm()} function in R is used to fit the generalized linear models:
\[ \text{\lstinline{fit = glm(formula, family = (link = ), data = )}}. \]
\begin{itemize}
    \item \lstinline{formula}: a linear formula describing the model, e.g.,
          \[ \text{\lstinline{resp \~ loc + clinic}}. \]
    \item \lstinline{family}: a description of the exponential family distribution and link
          function to be used in the model, e.g.,
          \[ \text{\lstinline{family = binomial, gaussian, poisson, Gamma, etc.}}. \]
          \[ \text{\lstinline{link = logit, log, loglog, cloglog, identity, probit, etc.}}. \]
...
\hlcom{# construct the binomial response for the logistic}
...
\end{knitrout}
% \end{noindent}
The logistic regression models are fit using the \lstinline{glm()} commands like:
% \begin{noindent}
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\end{knitrout}
% \end{noindent}
Components of the \lstinline{summary()} output for \lstinline{glm} objects:
\begin{itemize}
    \item \textcolor{Blue}{\lstinline{Estimate}}: the maximum likelihood estimates of the regression coefficients $ \hat{\beta}_0 $
              and $ \hat{\beta}_1 $.
    \item \textcolor{Blue}{\lstinline{Std. Error}}: estimated standard errors, the square root of the diagonal
    ...
    \item \textcolor{Blue}{\lstinline{z value}}: Wald-type test statistics for testing the hypotheses:
          \begin{center}
              $ \HN $: $ \beta_j=0 $ vs $ \HA $: $ \beta_j\ne 0 $.
          \end{center}
    \item \textcolor{Blue}{\lstinline{Pr(>|z|)}}: $ p $-value for above Wald test.
\end{itemize}
For this model:
\begin{itemize}
...
\begin{itemize}
    ...
    \item The \lstinline{summary()} output gives the test statistics and $p$-values for testing
          \begin{center}
              $ \HN $: $ \beta_j=0 $ vs $ \HA $: $ \beta_j\ne 0 $.
          \end{center}
\end{itemize}
...
\begin{itemize}
    ...
    \item $ \se{\hat{\beta}_1+\hat{\beta}_3} $ is required for calculation of \qty{95}{\percent} CI.
          ...
          \end{itemize}
    \item Therefore, $ \log{\psi}=\beta_1+\beta_3=\Vector{c}\Vector{\beta} $, $ \Vector{c}=(0,1,0,1)^\top $. In R, \lstinline{vcov(model3)}
...
\begin{itemize}
      \item Estimation: $ \hat{\Vector{\beta}} $ come from Fisher scoring using R function \lstinline{glm()}.
      ...
      \item Maximization of log-likelihood $ \ell(\Vector{\beta}) $ gives MLE $ \hat{\Vector{\beta}} $, and
            \begin{itemize}
                  \item estimated probability of response:
                        \[ \hat{\pi}_i=e^{\Vector{x}_i^\top \hat{\Vector{\beta}}}/(1+e^{\Vector{x}_i^\top \hat{\Vector{\beta}}})=\expit{\Vector{x}_i^\top \hat{\Vector{\beta}}}, \]
...
\begin{itemize}
      \item Model 2: Main Effects Model,
            ...
            \end{itemize}
      \item In R, the \lstinline{summary()} output $ D $ is reported as the \textcolor{Red}{Residual Deviance}.
            % \begin{noindent}
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\subsection*{Example: Prenatal Care Data}
\begin{itemize}
      \item Summary of Deviance (``\lstinline{residual deviance}'') from R output:
            \begin{table}[!htbp]
                  \centering
                  \begin{tabular}{clccc}
                        \toprule
...
\begin{itemize}
      ...
      \item This is reported as the ``\lstinline{Residual Deviance}'' in R \lstinline{glm} summary output.
