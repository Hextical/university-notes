...
\usepackage[activate={true,nocompatibility},final,tracking=true,factor=1100,stretch=10,shrink=10,babel]{microtype}
...
\begin{align*}
    ...
    \pdv{\theta}{\mu}   & =\biggl(\pdv{\mu}{\theta}\biggr)^{\!-1}=\frac{1}{b^{\prime\prime}(\theta)}, \\
...
\begin{align*}
    \pdv{\ell}{\beta_j}
     ...
     & =\frac{y-\mu}{\Var{Y}}\pdv{\mu}{\eta}x_j                                                  \\
     & =\frac{y-\mu}{\Var{Y}}\biggl(\pdv{\mu}{\eta}\biggr)^{\!2}\pdv{\eta}{\mu}x_j               \\
     & =(y-\mu)\Biggl(\Var{Y}\biggl(\pdv{\mu}{\eta}\biggr)^{\!2}\Biggr)^{-1}\pdv{\eta}{\mu}x_j   \\
...
\begin{itemize}
    ...
    \item $ \Matrix{X}=(x_1,\ldots,x_n) $ is a $ (p+1)\times n $ matrix,
    \item $ \MatrixCal{W}=\diag{W_1,\ldots,W_n} $, and
...
\begin{itemize}
    ...
    \item Can use Fisher Scoring (also known as IRWLS) to estimate the regression parameters $ \Vector{\beta} $ from any GLM based on general forms for $ \Matrix{I}(\Vector{\beta}) $
          and $ S(\Vector{\beta}) $.
    \item Practice: Chapter 1 review problems.
\end{itemize}
...
\[ Y_k \sim \Bin{m_k,\pi_k},\qquad k=1,2, \]
...
\[ \begin{array}{ccccc}
        \pi_1=\pi_2 & \implies & \OR=1   & \implies & \text{equal risk}             \\
...
\begin{Regular}{Wald Statistic}
    For a vector $ \Vector{\theta}=(\theta_1,\theta_2) $ where $ \theta_1=\log{\psi} $ is a scalar parameter of interest:
...
\begin{itemize}
    \item Calculation of $ I^{11} $ by using a general result:
          \[ I=\begin{pmatrix}
                  I_{11} & I_{12} \\
                  I_{21} & I_{22}
              \end{pmatrix},\qquad I^{-1}=\begin{pmatrix}
...
\end{itemize}
\subsection*{Confident Interval for Odds Ratio}
Here, we obtain:
...
\begin{itemize}
    ...
    \item A \qty{95}{\percent} CI for odds ratio $ \psi $:
          ...
          hypothesis of no association between fetal mortality and level of care.
\end{itemize}
\subsection*{Example: Prenatal Care from Two Clinics}
There is an \textcolor{Blue}{additional explanatory variable}: Clinic (A vs B). The association between
Mortality and Level of Care (by Clinic).
TODOtab2
\begin{itemize}
    \item $ \hat{\psi}_\text{A}=0.80\qquad(0.37,1.73) $ and $ \hat{\psi}_\text{B}=1.01\qquad(0.33,3.10) $.
    \item These results do NOT agree with the results from the pooled analysis on
          the previous slide.
\end{itemize}
The association between Level of Care and Clinic:
TODOtab3
\begin{itemize}
    \item $ \hat{\psi}=14.06\qquad(9.12,21.76) $.
\end{itemize}
The association between Mortality and Clinic
TODOtab4
\begin{itemize}
    \item $ \hat{\psi}=0.35\qquad(0.21,0.58) $.
\end{itemize}
\begin{itemize}
