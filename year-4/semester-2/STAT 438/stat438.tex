...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# Check the normal assumption for mother's weight concern:}
\hlstd{mydata} \hlkwb{<-} \hlkwd{read.csv}\hlstd{(}\hlstr{"data/diet.csv"}\hlstd{,} \hlkwc{sep} \hlstd{=} \hlstr{","}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in file(file, "{}rt"{}): cannot open the connection}}\begin{alltt}
\hlstd{mydata[}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{, ]}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in eval(expr, envir, enclos): object 'mydata' not found}}\begin{alltt}
\hlkwd{hist}\hlstd{(mydata}\hlopt{$}\hlstd{M2WTCON)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in hist(mydata\$M2WTCON): object 'mydata' not found}}\begin{alltt}
\hlkwd{qqnorm}\hlstd{(mydata}\hlopt{$}\hlstd{M2WTCON)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in qqnorm(mydata\$M2WTCON): object 'mydata' not found}}\begin{alltt}
\hlkwd{qqline}\hlstd{(mydata}\hlopt{$}\hlstd{M2WTCON)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in quantile(y, probs, names = FALSE, type = qtype, na.rm = TRUE): object 'mydata' not found}}\begin{alltt}
\hlcom{# Calculate the numerator and the denominator for the}
\hlcom{# weights}
\hlstd{ps.num} \hlkwb{<-} \hlkwd{dnorm}\hlstd{(mydata}\hlopt{$}\hlstd{M2WTCON,} \hlkwd{mean}\hlstd{(mydata}\hlopt{$}\hlstd{M2WTCON),} \hlkwd{sd}\hlstd{(mydata}\hlopt{$}\hlstd{M2WTCON))}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in dnorm(mydata\$M2WTCON, mean(mydata\$M2WTCON), sd(mydata\$M2WTCON)): object 'mydata' not found}}\begin{alltt}
\hlstd{model.den} \hlkwb{<-} \hlkwd{lm}\hlstd{(M2WTCON} \hlopt{~} \hlstd{.,} \hlkwc{data} \hlstd{= mydata[,} \hlopt{-}\hlnum{1}\hlstd{])}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in is.data.frame(data): object 'mydata' not found}}\begin{alltt}
\hlstd{ps.den} \hlkwb{<-} \hlkwd{dnorm}\hlstd{(mydata}\hlopt{$}\hlstd{M2WTCON, model.den}\hlopt{$}\hlstd{fitted, (}\hlkwd{summary}\hlstd{(model.den))}\hlopt{$}\hlstd{sigma)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in dnorm(mydata\$M2WTCON, model.den\$fitted, (summary(model.den))\$sigma): object 'mydata' not found}}\begin{alltt}
\hlstd{wts} \hlkwb{<-} \hlstd{ps.num}\hlopt{/}\hlstd{ps.den}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in eval(expr, envir, enclos): object 'ps.num' not found}}\begin{alltt}
\hlkwd{boxplot}\hlstd{(wts)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in boxplot(wts): object 'wts' not found}}\begin{alltt}
\hlcom{# Fit the MSM and obtain the robust standard error}
\hlstd{mydata}\hlopt{$}\hlstd{wts} \hlkwb{<-} \hlstd{wts}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in eval(expr, envir, enclos): object 'wts' not found}}\begin{alltt}
\hlstd{design} \hlkwb{<-} \hlkwd{svydesign}\hlstd{(}\hlkwc{ids} \hlstd{=} \hlopt{~}\hlnum{1}\hlstd{,} \hlkwc{weights} \hlstd{=} \hlopt{~}\hlstd{wts,} \hlkwc{data} \hlstd{= mydata)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in svydesign(ids = \textasciitilde{}1, weights = \textasciitilde{}wts, data = mydata): object 'mydata' not found}}\begin{alltt}
\hlkwd{svyglm}\hlstd{(earlydiet} \hlopt{~} \hlstd{M2WTCON,} \hlkwc{design} \hlstd{= design,} \hlkwc{family} \hlstd{=} \hlkwd{quasibinomial}\hlstd{())}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{Error in .svycheck(design): object 'design' not found}}\end{kframe}
\end{knitrout}
%\end{noindent}
Conclusion: for $1 $ unit increase in mother's weight concern, the
odds of the daughter dieting between ages 7 and 11 increases by
$\exp{0.9370} = 2.55$ times. The causal effect is significant.
\end{document}
