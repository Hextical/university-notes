...
%\begin{noindent}
    <<>>=
    # Check the normal assumption for mother's weight concern:
    mydata=read.csv("data/diet.csv",sep=",")
    mydata[1:5,]
    hist(mydata$M2WTCON)
    qqnorm(mydata$M2WTCON)
    qqline(mydata$M2WTCON)

    # Calculate the numerator and the denominator for the weights
    ps.num=dnorm(mydata$M2WTCON, mean(mydata$M2WTCON), sd(mydata$M2WTCON))
    model.den=lm(M2WTCON~.,data=mydata[,-1])
    ps.den=dnorm(mydata$M2WTCON, model.den$fitted, (summary(model.den))$sigma)
    wts=ps.num/ps.den
    boxplot(wts)
    # Fit the MSM and obtain the robust standard error
    mydata$wts=wts
    design <- svydesign(ids=~1, weights=~wts, data=mydata)
    svyglm(earlydiet ~ M2WTCON , design=design,family=quasibinomial())
    @
%\end{noindent}
