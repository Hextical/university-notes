...
\textrightarrow{} \code{a.out} executable

\code{\#include} is a direct copy/paste for the preprocessor directive

\code{\#include} with quotes \textrightarrow{} look in current directory,
...
\begin{itemize}
      \item \code{\#define VAR VALUE} \textrightarrow{} searches and replaces
            \code{VAR} with \code{VALUE}
            \begin{itemize}
                  \item \code{\#define MAX 10} \textrightarrow{} replaces \code{MAX} with \code{10}
...
\code{g++14 -D VAR=VALUE FILE} \textrightarrow{} changes type in command-line
for \code{course.cc}
...
\code{g++14 -DDEBUG debug.cc} \textrightarrow{} defines \code{DEBUG}, so
...
\code{g++14 -c vec.cc}, \code{g++14 -c main.cc} \textrightarrow{} compiles each
separately, without the executable (\code{.o} (Object) files are produced).

\code{g++14 main.o vec.o -o myprog} \textrightarrow{} merges them into
...
File: \code{example1}
\begin{itemize}
      \item \code{.PHONY} \textrightarrow{} specifies that \code{clean} is not a file, but
            a command
            \begin{itemize}
                  \item \code{clean} \textrightarrow{} \code{make clean} will delete any \code{.o} files
...
\begin{itemize}
      ...
      \item \code{-Wall} \textrightarrow{} warn all, compiler will give errors for warnings
\end{itemize}
