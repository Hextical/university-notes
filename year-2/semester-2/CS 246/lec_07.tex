...
\begin{itemize}
    \item Default arguments must come last, e.g. \code{void test(int x = 0, string str);}
          \textrightarrow{}  illegal because \code{string} has no default value but comes last
    \item \code{void test(int x = 0, string str = ``hello'');} \textrightarrow{} legal,
    ...
    \item When calling the function, we can use default values for the last
          ...
          \begin{itemize}
              \item \code{test();} \textrightarrow{} legal;
              \item \code{test(s);} \textrightarrow{} legal;
              \item \code{test(s, ``bla'');} \textrightarrow{} legal;
              \item \code{test(\textvisiblespace,``bla'')} \textrightarrow{} illegal;
              \item \code{test(``bla'');} \textrightarrow{} illegal;
          \end{itemize}
    ...
    \item A new function: \code{void test(int, int);} \textrightarrow{} legal,
          does not conflict with \code{test(int, string);}
    \item A new function: \code{void test(string);} \textrightarrow{} legal
\end{itemize}
...
\begin{itemize}
    ...
    \item C++ allows us to define the meaning of operators for user-define types
          (all types not built-in); \code{istream} \textrightarrow{} \code{iostream}
          header.
\end{itemize}
\begin{lstlisting}
    int x;
...
\begin{itemize}
    ...
    \item \code{const Node n\{3, nullptr\}} \textrightarrow{} \code{3} data,
          \code{nullptr} next
    \item \code{n.data} or \code{n.next} \textrightarrow{} illegal
\end{itemize}
\begin{lstlisting}
    int n = 5;
    const int *p = &n;
...
\begin{itemize}
    \item \code{p} is a pointer to an \code{int} which is \code{const}; pointer
          ...
          \begin{itemize}
              \item \code{p = \&m;} \textrightarrow{} legal
              \item \code{*p = 10;} \textrightarrow{} illegal, but can still do \code{n = 10;}
          \end{itemize}
    \item \code{int *const q = \&n;} \textrightarrow{} \code{q} is a \code{const}
          ptr to an \code{int}
          \begin{itemize}
              \item \code{q = \&m;} \textrightarrow{} illegal
              \item \code{*q = 10;} \textrightarrow{} legal
          \end{itemize}
    \item \code{const int *const r = \& n;} \textrightarrow{} r is a \code{const int}
...
an assignment.
\begin{itemize}
    \item \code{x = 5;} \textrightarrow{} \code{x} is an lvalue
    \item \code{5 = 7} \textrightarrow{} not an lvalue
    \item \code{x + y = 5} \textrightarrow{} not an lvalue
    \item \code{str[i] = '3';} \textrightarrow{} \code{str[i]} lvalue
\end{itemize}
...
\begin{itemize}
    ...
    \item \code{z = 20;} \textrightarrow{} automatically dereferencing,
          don't need \code{*z} (actually a compile error); changes \code{y} to \code{20}
    \item \code{z} becomes an alias to \code{y}
    \item \code{int *p = \&z} \textrightarrow{} gets \code{y}'s address
    \item \code{z} behaves like \code{y}
    \item \code{int \&} is a type, \textbf{not} an address
\end{itemize}
References must be initialized to lvalues.
\begin{itemize}
    \item \code{int \&z;} \textrightarrow{} illegal
    \item \code{int \&z = 3;} \textrightarrow{} illegal
    \item \code{int \&z = a + b;} \textrightarrow{} illegal
\end{itemize}
\begin{itemize}
...
\begin{lstlisting}
    ...
    cout << x; // prints 6
\end{lstlisting}
\code{cin >{}> x} \textrightarrow{} \code{x} is passed by reference
