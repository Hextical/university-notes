...
\myuline{5.6 Geometric Distribution}
...
\myuline{Find the probability function, $ f(x)$}

Substitute $ k=1 $ into the Negative Binomial distribution to obtain
\[ f(x)=p(1-p)^x \]
for $ x\in[0,\infty) $.

\myuline{Prove the following:}
...
\myuline{Find the cumulative distribution function, $ F(x)$}
\begin{align*}
    F(x) & =P(X\le x)                                         \\
         & =1-P(X>x)                                          \\
