...
\begin{Theorem}{Linderberg-Feller CLT for Triangular Arrays}{}
    ...
    \[ \frac{1}{\sigma_i^2}\sum_{j=1}^{n_i} \E{X_{i,j}^2
            \Ind{\abs{X_{i,j}}>\varepsilon\sigma_i}}\to 0
        \text{ as } i\to\infty \]
    then
...
\begin{Proof}{\Cref{thm:m_clt}}{pf_clt}
    ...
    \[
        ...
        N\E{Y_{1}^2 \Ind{\abs{Y_{1}}>\varepsilon\sigma_N}} \to 0\text{ as } T\to\infty
    \]
    \emph{Aside}:
    \begin{align*}
        \E[\big]{\abs{Y}^{2+\delta}}
...
\begin{Theorem}{Linear Process CLT}{lin_clt}
    ...
    then
    \[ S_T\stackrel{D}{\to}\N[\bigg]{0,\sum_{\ell=-\infty}^{\infty} \gamma(\ell)}\text{ as }
        T\to\infty \]
\end{Theorem}
...
\begin{Example}{}{}
    ...
    \[ \frac{\hat{\rho}(h)}{T^2}\to C\quad(\forall h,\, T\to\infty)  \]
    Hence
...
Looking back at~\Cref{fig:gtemp}, we see that this time series has an
...
acf(gtemp)
\end{minted}
Clearly,~\Cref{fig:gtemp_acf} is indicative
of a strong trend or non-stationarity.

TODO, 2 figures

Similar pattern indicative of non-stationarity.
