...
\begin{enumerate}[(1)]
    \item Use a $ \GARCH{1,1} $ model. ``We do not find much evidence
          that the $ \GARCH{1,1} $ model is outperformed.'' ---Hansen and Lunde (2001).
...
\begin{Remark}{Cross-validation}{}
    ...
    where $ \hat{\sigma}_t^2 $ is estimated from $ X_{t-1},\ldots,X_1 $. It is
...
\begin{Definition}{Horizon $ h $ loss}{}
    ...
    \[ L_{t,t+h}=-(V_{t+h}-V_t) \]
    where $ V_{t+h}-V_t $ is known as the horizon $ h $ return.
\end{Definition}
...
\end{Definition}
In practice, we take
\[ \text{VaR}_{t,h}(\alpha)=\text{inf}\{x: \Prob{L_{t,h}>x\given\mathcal{F}_t}\le \alpha\} \]
...
\begin{Remark}{}{}
    ...
    \[ \Prob{L_{t,h}>\text{VaR}_{t,h}(\alpha)}=\alpha \]
\end{Remark}
\begin{Example}{}{}
...
\begin{Remark}{}{}
    ...
    \[ L_{t,t+h}=-\sum_{j=t+1}^{t+h}r_j\quad[Telescoping Sum]  \]
...
\end{Remark}
\begin{Definition}{Risk metrics model}{}
    Let $ r_t $ denote the returns (or log-returns). The
    \textbf{Risk Metrics model} is defined by
    \begin{flalign*}
...
\begin{Remark}{}{}
    \begin{enumerate}[(1)]
        ...
        \item The Risk Metrics model leads to a degenerate GARCH model ($ \omega=0 $). It tends
              to underestimate $ \sigma_{t}^2 $.
    \end{enumerate}
\end{Remark}
\subsection*{A General Approach Using GARCH Models}
\begin{itemize}
    \item Step 1: Fit a GARCH model to returns $ r_t $.
...
\begin{itemize}
    \item Option 1: Appoly $ \sqrt{h} $-scaling.
...
\begin{Definition}{Backtesting}{}
    \textbf{Backtesting} returns to the practice of testing predictive
...
\begin{itemize}
    ...
    \item ``Tightness/Sharpness to the Data:'' If
          \[ \Prob*{L_{t,t+h}>\text{VaR}^\prime_{t,h}(\alpha)}=\Prob*{L_{t,t+h}>\text{VaR}^2_{t,h}(\alpha)} \]
...
\begin{itemize}
    ...
    \item Risk Metrics: $ \hat{q}_t(1,\alpha)=\hat{\sigma}_{t+1}\Phi^{-1}(\alpha) $,
          $ \hat{\sigma}_{t+1} $ coming from the Risk Metrics ``recursion'' with $ \lambda=0.94 $
...
\begin{align*}
    \Var{\bar{X}}
     ...
     & \approx \frac{1}{T} \sum_{h=-\infty}^{\infty} \gamma_X(h)\text{as }T\to\infty
\end{align*}
...
\begin{Theorem}{}{}
    If $ \set{X_t}_{t\in\mathbf{Z}} $ is strictly stationary and sutiably weakly
    dependent, then
...
\end{itemize}
\subsection*{Kwiatowski-Phillips-Schmidt-Shin (KPSS) Test}
Consider
\[ Z_T(x)=\frac{1}{\sqrt{T}} \sum_{i=1}^{\lfloor T_x \rfloor} (X_i-\bar{X})=S_T-\frac{\lfloor T_x\rfloor}{T}S_T(1)  \]
where $ S_T(x)=\frac{1}{\sqrt{T}} \sum_{t=1}^{\lfloor T_x\rfloor} (X_i-\mu) $. As we mentioned,
...
\end{Remark}
Diebold-Mariano (1995) suggest testing
