    ...
    \begin{center}
        \begin{tikzpicture}
            \begin{axis}[ylabel = $y$, xlabel = $x$]
                ...
                \addplot[blue, domain=0:1.25, name path=F] {sqrt(\x)} node [pos = 1, above right] {$y=\sqrt{x}$};
                \addplot[red, domain=0:1.25, name path=G] {x^2} node [pos = 0.9, above right] {$y=x^2$};
                \addplot[black, domain=0:1.5] {1};
                \draw[purple, very thick, dotted] (0.3,0.3^2) -- (0.3,1);
                \draw[green, very thick, dotted] (0.3,0.5477) -- (0.3,1);
            \end{axis}
        \end{tikzpicture}
    \end{center}
