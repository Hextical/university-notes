...
\begin{Theorem}{Properties of Integrals}{properties_of_integrals}
    If $ f $ is integrable on $ \interval{a}{b} $, then:
    \begin{enumerate}[label=(\arabic*)]
        \item\label{property_integral_1} For any $ c\in\mathbb{R} $,
              \[ \int_{a}^{b} cf(x)\, d{x} = c \int_{a}^{b} f(x)\, d{x} \]
        \item\label{property_integral_2}
              \[ \int_{a}^{b} (f+g)(x)\, d{x} = \int_{a}^{b} f(x)\, d{x} +
                  \int_{a}^{b} g(x)\, d{x} \]
        \item\label{property_integral_3} If $ m\le f(x)\le M $ for $ x\in\interval{a}{b} $, then
              \[ m(b-a)\le \int_{a}^{b} f(x)\, d{x} \le M(b-a) \]
        \item\label{property_integral_4} If $ 0\le f(x) $ for $ x\in\interval{a}{b} $, then
              \[ 0\le \int_{a}^{b} f(x)\, d{x} \]
        \item\label{property_integral_5} If $ f(x)\le g(x) $ for $ x\in\interval{a}{b} $, then
              \[ \int_{a}^{b} f(x)\, d{x} \le \int_{a}^{b} g(x)\, d{x} \]
        \item\label{property_integral_6} If $ \abs{f} $ is integrable on $ \interval{a}{b} $, then
              \[ \abs[\bigg]{\int_{a}^{b} f(x)\, d{x}}
...
So, in general, $ \int_{a}^{b} $ is the area under $ f $ that
lies above the $ x $-axis \textbf{minus} the area above the graph of
